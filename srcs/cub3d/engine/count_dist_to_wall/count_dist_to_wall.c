...
t_distance	dist_to_texture_init(t_distance dist_to_wall, float pov, float angle)
{
	t_distance	distance;

	distance.x = modff(dist_to_wall.x / CELL / 2, &dist_to_wall.x);
	distance.distance = dist_to_wall.distance * cosf(pov - angle);
	return (distance);
}

t_distance	count_dist_to_wall(t_cub *cub, t_player player, float ray_angle, t_distance *distance)
{
	t_screen	screen = cub->screen;
	t_distance	dist_to_wall_h;
	t_distance	dist_to_wall_v;
	t_distance	dist_to_item_h;
	t_distance	dist_to_item_v;

	dist_to_wall_h = dist_to_wall_horizontal(player, ray_angle, screen.width, &dist_to_item_h);
	dist_to_wall_v = dist_to_wall_vertical(player, ray_angle, screen.height, &dist_to_item_v);
	get_wall_texture(cub, ray_angle, dist_to_wall_h.distance, dist_to_wall_v.distance);
	if (dist_to_item_h.distance > dist_to_item_v.distance)
		*distance = dist_to_texture_init(dist_to_item_v, player.pov, ray_angle);
	else
		*distance = dist_to_texture_init(dist_to_item_h, player.pov, ray_angle);
...
t_distance	count_distance(t_cub *cub, float ray_angle, t_distance *dist_to_item)
{
	...
	dist_to_item->distance = -1;
	dist_to_item->x = -1;
	dist_to_wall = count_dist_to_wall(cub, player, ray_angle, dist_to_item);
	return (dist_to_wall);
}
